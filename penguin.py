...
def main(argv):
	...
	for i in range(0,N,10):
		year = i
		percent = (CEPD[i]*100)
		plt.plot(year,percent,"bo")
	plt.title("CEPD run #1 of 1000 simulations for 201 years with the default parameters")
	plt.xlabel("Year")
	plt.ylabel("CEPD")
	plt.show()
	
	#plot for the 3 year El Nino cycle
	if enYears == 3:
		for i in range (0,N,10):
			year = i
			percent = (CEPD[i]*100)
			plt.plot(year,percent,"ro")
		plt.title("CEPD vs. Year for a 3 Year El Nino Cycle")
		plt.xlabel("Year")
		plt.ylabel("CEPD")
		plt.show()
	
	#plot for the 5 year El Nino cycle
	if enYears == 5:
		for i in range (0,N,10):
			year = i
			percent = (CEPD[i]*100)
			plt.plot(year,percent,"bs")
		plt.title("CEPD vs. Year for a 5 Year El Nino Cycle")
		plt.xlabel("Year")
		plt.ylabel("CEPD")
		plt.show()
	
	#plot for the 7 year El Nino cycle
	if enYears == 7:
		for i in range (0,N,10):
			year = i
			percent = (CEPD[i]*100)
			plt.plot(year,percent,"g^")
		plt.title("CEPD vs. Year for a 7 Year El Nino Cycle")
		plt.xlabel("Year")
		plt.ylabel("CEPD")
		plt.show()
